...
private:
	int_errno RollDice(std::string dice) const
	{
		...
		if (dice[0] == 'P')
		{
			...
			vintTmpRes.push_back(intD100Res);
			while (intPNum--)
			{
...
public:
	...
	int_errno Max() const
	{
																											...
																											npos)
		{
			...
			if (*(vboolNegative.end() - 1))
				intRDRes = MinDice(strtemp);
			else
				intRDRes = MaxDice(strtemp);
			intReadDiceLoc = intSymbolPosition + 1;
			if (dice[intSymbolPosition] == '+')
				vboolNegative.push_back(false);
			else
	...
	std::string FormStringSeparate() const
	{
		...
		for (auto i = vvintRes.begin(); i != vvintRes.end(); ++i)
		{
			...
			if (vBnP[distance(vvintRes.begin(), i)] == Normal_Dice)
			{
				...
				{
					strReturnString += "*" + std::to_string(vintMultiplier[distance(vvintRes.begin(), i)]);
				}
			}
...
extern inline void BG(std::string &);

extern inline void BGC(std::string &);

extern inline void NPCGroup(std::string &);

extern inline void NPCKP(std::string &);

extern inline void GAS(std::string &);

extern inline void TZ(std::string &);
